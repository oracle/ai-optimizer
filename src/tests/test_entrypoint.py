...
class TestStartClient:
    ...
    def _clear_auth_gateway_environment(self, monkeypatch):
        for name in (
            ...
            "AIO_AUTH_DEV_WEB_CLIENT_SECRET",
            "AIO_AUTH_DEV_ADMIN_PASSWORD",
        ):
    ...
    def test_with_pydantic_ssl_value(self, tmp_path, monkeypatch):
        """start_client should accept the same true spellings as Pydantic settings."""
        monkeypatch.setenv("AIO_CLIENT_SSL", "yes")
        from unittest.mock import patch
...
"""
    ...
    def test_operator_development_oidc_configuration_is_not_overwritten(self, tmp_path, monkeypatch):
        ...
        config_dir.mkdir(parents=True)
        original = """[auth]
redirect_uri = "https://operator.example.test/oauth2callback"
cookie_secret = "operator-cookie-secret"
client_id = "platform-web-client"
client_secret = "launcher-web-client-secret"
server_metadata_url = "http://127.0.0.1:8765/.well-known/openid-configuration"
client_kwargs = { scope = "openid profile email aio.api" }
expose_tokens = "access"
"""
        ...
        assert (config_dir / "secrets.toml").read_text(encoding="utf-8") == original

    def test_non_string_development_oidc_redirect_is_left_unchanged(self, tmp_path, monkeypatch):
        """A non-string callback value must not crash or be treated as generated."""
        (tmp_path / "server").mkdir()
        config_dir = tmp_path / "client" / "app" / ".streamlit"
        config_dir.mkdir(parents=True)
        original = """[auth]
...
cookie_secret = "operator-cookie-secret"
client_id = "platform-web-client"
client_secret = "operator-web-client-secret"
server_metadata_url = "http://127.0.0.1:8765/.well-known/openid-configuration"
client_kwargs = { scope = "openid profile email aio.api" }
expose_tokens = "access"
"""
    ...
    def test_development_oidc_explicit_redirect_overrides_client_listener(self, tmp_path, monkeypatch):
        ...
        contents = (tmp_path / "client" / "app" / ".streamlit" / "secrets.toml").read_text(encoding="utf-8")
        assert 'redirect_uri = "https://optimizer.example.test/oauth2callback"' in contents
