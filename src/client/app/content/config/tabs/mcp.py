...
# Helpers
###################################
def _configs_lookup(section: str, key: str) -> dict:
    """Convert state.mcp_configs[section] into a lookup keyed by *key*."""
    return {c[key]: c for c in (state.mcp_configs.get(section) or []) if key in c}
...
def get_mcp_status() -> dict:
    """Get MCP Status"""
    try:
        return api_get("healthz", api_prefix="/mcp")
...
def get_mcp(force: bool = False) -> None:
    ...
    if force or "mcp_configs" not in state or not state.mcp_configs:
        ...
        for key, endpoint in endpoints.items():
            try:
                results[key] = api_get(endpoint, api_prefix="/mcp")
            except httpx.HTTPStatusError as ex:
...
def mcp_details(mcp_server: str, mcp_type: str, mcp_name: str) -> None:
    ...
    if config.get("text"):
        st.subheader("Instructions", divider="red")
        st.code(config["text"], wrap_lines=True, height="content")
...
def display_mcp() -> None:
    ...
    if not selected_mcp_server:
        return
    if state.mcp_configs["tools"]:
        tools_lookup = _configs_lookup("tools", "name")
    ...
    if state.mcp_configs["prompts"]:
        prompts_lookup = _configs_lookup("prompts", "name")
    ...
    if state.mcp_configs["resources"]:
        resources_lookup = _configs_lookup("resources", "name")
