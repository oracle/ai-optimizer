...
_GENERATED_OIDC_CONFIG_MARKER = "# Generated by the All-In-One authentication launcher"
...
def start_client(script_dir: Path) -> None:
    ...
    _prepare_auth_streamlit_config(script_dir, cookie_secret)
...
def _prepare_auth_streamlit_config(script_dir: Path, cookie_secret: str) -> None:  # noqa: PLR0912
    """Create Streamlit OIDC settings for the embedded authentication gateway."""
    if not _uses_embedded_auth_gateway(script_dir):
        return
    if not os.environ.get("AIO_AUTH_LOCAL_ADMIN_PASSWORD") and os.environ.get("AIO_AUTH_DEV_ADMIN_PASSWORD"):
        os.environ["AIO_AUTH_LOCAL_ADMIN_PASSWORD"] = os.environ["AIO_AUTH_DEV_ADMIN_PASSWORD"]
    if not os.environ.get("AIO_AUTH_WEB_CLIENT_SECRET") and os.environ.get("AIO_AUTH_DEV_WEB_CLIENT_SECRET"):
        os.environ["AIO_AUTH_WEB_CLIENT_SECRET"] = os.environ["AIO_AUTH_DEV_WEB_CLIENT_SECRET"]
    if _is_local_all_in_one(script_dir):
        bootstrap_credential = None
        if os.environ.get("AIO_AUTH_MODE", "local") == "local" and not os.environ.get("AIO_AUTH_LOCAL_ADMIN_PASSWORD"):
            bootstrap_credential = load_or_create_bootstrap_credential(script_dir)
            os.environ["AIO_AUTH_LOCAL_ADMIN_PASSWORD"] = bootstrap_credential.password
        if not os.environ.get("AIO_AUTH_WEB_CLIENT_SECRET"):
            os.environ["AIO_AUTH_WEB_CLIENT_SECRET"] = load_or_create_web_client_secret(
                script_dir,
    ...
    issuer = (
        os.environ.get("AIO_AUTH_ISSUER") or os.environ.get("AIO_AUTH_DEV_ISSUER") or "http://127.0.0.1:8765"
    ).rstrip("/")
    configured_redirect_uri = os.environ.get("AIO_AUTH_WEB_REDIRECT_URI") or os.environ.get(
    ...
    if configured_redirect_uri is None or not configured_redirect_uri.strip():
        ...
        os.environ["AIO_AUTH_WEB_REDIRECT_URI"] = redirect_uri
        if os.environ.get("AIO_AUTH_MODE") == "dev":
            os.environ["AIO_AUTH_DEV_WEB_REDIRECT_URI"] = redirect_uri
    else:
        redirect_uri = configured_redirect_uri
    web_client_secret = os.environ["AIO_AUTH_WEB_CLIENT_SECRET"]
    if secrets_file_exists:
        ...
        generated_config = (
            ...
            and auth_config.get("client_kwargs")
            in ({"scope": "openid profile email"}, {"scope": "openid profile email aio.api"})
            and auth_config.get("expose_tokens") == "access"
            and (
                _GENERATED_OIDC_CONFIG_MARKER in contents
                or contents.startswith("# Generated by the All-In-One")
                or (
...
def _uses_embedded_auth_gateway(script_dir: Path) -> bool:
    """Return whether this client launch uses an embedded authentication gateway."""
    auth_mode = os.environ.get("AIO_AUTH_MODE")
    if auth_mode in {"local", "github", "oidc"}:
        return True
    if auth_mode == "dev":
        return _is_local_all_in_one(script_dir)
