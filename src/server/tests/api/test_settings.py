...
async def test_get_settings_includes_sensitive(app_client, auth_headers):
    ...
    resp = await app_client.get(
        '/v1/settings', params={'include_sensitive': 'true'}, headers=auth_headers
    )
    assert resp.status_code == 200
    body = resp.json()
...
def test_sensitive_fields_derived_from_models():
    ...
    assert SENSITIVE_FIELDS['database_configs']['__all__'] == set(
        DatabaseSensitive.model_fields.keys()
    )
    assert SENSITIVE_FIELDS['oci_profile_configs']['__all__'] == set(
        OciSensitive.model_fields.keys()
    )
