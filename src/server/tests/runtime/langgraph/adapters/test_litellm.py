...
class TestClientParamsOverrides:
    ...
    @patch("server.app.runtime.langgraph.adapters.litellm.ChatLiteLLM.completion_with_retry")
    def test_oci_openai_gpt5_drops_default_null_max_tokens(self, mock_completion):
        """With no max_tokens configured, GPT-5/o-series must not receive ``max_tokens=None``.

        ``ChatLiteLLM._default_params`` always carries ``max_tokens`` (None when
        unset); leaving an explicit ``max_tokens=None`` in the request undermines
        the GPT-5 compatibility fix.
        """
        mock_completion.return_value = _non_streaming_response("ok")
        llm = OracleChatLiteLLM(model="oci/openai.gpt-5.5")
        llm._generate([HumanMessage(content="hi")])
