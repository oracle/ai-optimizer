# Adapted from Giskard 2.19.2, giskard/rag/report.py.
# Copyright Giskard AI. Licensed under Apache-2.0; see LICENSE.giskard.
# Modified for AI Optimizer: report plots use their fixed Bokeh colors.
...
from giskard.rag.question_generators import COMPONENT_DESCRIPTIONS, QUESTION_ATTRIBUTION, RAGComponents
from giskard.rag.testset import QATestset, QuestionSample
from giskard.visualization.widget import get_template

try:
    from bokeh.document import Document
    from bokeh.embed import components
    from bokeh.io import curdoc, output_notebook, reset_output
    from bokeh.models import ColumnDataSource, DataRange1d, FactorRange, Span, TabPanel, Tabs, Title
...
class RAGReport:
    ...
    model_outputs : Sequence[AgentAnswer]
        The agent's answers, in testset order.
    metrics_results : dict, optional
        Metric values keyed by question ID, including correctness and its reason.
    ...
    def save(self, folder_path: str):
        ...
        if self._knowledge_base is not None:
            self._knowledge_base._knowledge_base_df.to_json(  # noqa: SLF001
    ...
    def load(
        ...
        with open(path / "agent_answer.json", "r", encoding="utf-8") as source:
            answers = json.load(source)
        model_outputs = [AgentAnswer(**answer) for answer in answers]

        with open(path / "report_details.json", "r", encoding="utf-8") as source:
            report_details = json.load(source)
        testset_frame = testset.to_pandas()
        testset_frame.index = testset_frame.index.astype(str)

        knowledge_base = None
        if (path / "knowledge_base.jsonl").exists():
            with open(path / "knowledge_base_meta.json", "r", encoding="utf-8") as source:
                knowledge_base_meta = json.load(source)
            ...
            knowledge_base._topics_inst = topics  # noqa: SLF001

            for doc_idx, doc in enumerate(knowledge_base._documents):  # noqa: SLF001
                doc.topic_id = documents_topics[doc_idx]

        metrics_results = {}
        if (path / "metrics_results.json").exists():
            with open(path / "metrics_results.json", "r", encoding="utf-8") as source:
                metrics_results = json.load(source)
    ...
    def component_scores(self) -> pd.DataFrame:
        ...
        scores = {
            component: (
                [
                    sum(
                        1 / len(attribution) * cast(float, correctness.loc[q_type, "correctness"])
                        for q_type in attribution
                    )
                ]
    ...
    def _get_knowledge_plot(self):
        ...
        # Giskard 2.19.2 annotates dicts here, but its plot consumes QuestionSample attributes.
        get_failure_plot = cast(Callable[[Sequence[QuestionSample]], figure], self._knowledge_base.get_failure_plot)
        tabs = [
            TabPanel(child=self._knowledge_base.get_knowledge_plot(), title="Topic exploration"),
            TabPanel(
                child=get_failure_plot(
                    [
                        QuestionSample(
                            **cast(dict[str, Any], question), id="", reference_context="", conversation_history=[]
                        )
    ...
    def plot_correctness_by_metadata(self, metadata_name: str):
        ...
        p = figure(
            y_range=FactorRange(factors=metadata_values),
            x_range=DataRange1d(start=0),
            title=Title(text_font_size="14pt"),
            height=350,
    ...
    def plot_metrics_hist(self, metric_name: str, filter_metadata: Optional[dict] = None):
        """
        ...
        filter_metadata : dict, optional
            Filter the dataframe by metadata names and their allowed values.
        """
        if metric_name in self.metric_names:
            ...
            p = figure(
                ...
                height=200,
                toolbar_location=None,
                title=Title(text=metric_name.replace("_", " "), text_font_size="12pt"),
                tools="hover",
                width_policy="max",
            )
