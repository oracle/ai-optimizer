...
def db_config_to_entry(state: DatabaseState) -> DatabaseConfigEntry:
    """Convert internal DatabaseState to a JSON-serializable entry."""
...
def registry_to_persisted(
    registry: list[DatabaseState],
    active_alias: str = "CORE",
    oci_configs: list | None = None,
) -> PersistedSettings:
    """Build PersistedSettings from the current registry state.

    *oci_configs* should be pre-serialized (list of dicts) so this module
    does not need to import from the ``oci`` package.
    """
