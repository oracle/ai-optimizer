...
| `config.py` | OCI config file parser (`parse_oci_config_file`) and connectivity check (`_check_usable`) |
...
| `usable` | `bool?` | `False` | Set at runtime by `_check_usable` |
...
5. `load_oci_profiles()` called:
   ...
   - Each profile is tested via `_check_usable()`.
...
## Connectivity Check (`_check_usable`)
...
- On success: sets `profile.usable = True`, returns `None`.
- On failure: sets `profile.usable = False`, returns the error string.
...
| `resource_principal` | `get_resource_principals_signer()` &mdash; for OCI Functions and resource-principal-enabled services |
...
| `POST` | `/` | Create a profile. Runs `_check_usable`; persists even if connectivity fails (422), 409 if duplicate. |
| `PUT` | `/{auth_profile}` | Update a profile. Runs `_check_usable`; rolls back changes if the profile was previously usable and the check fails (422). |
| `DELETE` | `/{auth_profile}` | Remove a profile. 404 if not found. |

## Persist-on-Failure Semantics

Both **create** (`POST`) and **update** (`PUT`) persist the profile before
checking usability. If `_check_usable` fails the endpoint returns **422**, but
the profile is still saved (with `usable=False`). This is intentional:

- Users can inspect the saved profile, correct credentials or network issues,
  and retry without re-entering all fields.
- The `PUT` endpoint additionally rolls back field changes when the profile was
  *previously* usable, so a bad edit does not silently downgrade a working
  profile.
- The `usable` flag lets clients distinguish healthy profiles from ones that
  need attention.
