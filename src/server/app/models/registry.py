...
def register_model(model: ModelConfig) -> None:
    ...
    if not model.id or not model.provider:
        LOGGER.warning("Skipping model registration: missing id or provider")
        return
    key = _model_key(model.id, model.provider)
    settings.model_configs = [
        m for m in settings.model_configs if not m.id or not m.provider or _model_key(m.id, m.provider) != key
    ] + [model]


def reset_factory_models() -> None:
    """Clear model_configs and re-register every FACTORY_MODELS entry."""
    settings.model_configs = []
    for entry in FACTORY_MODELS:
        register_model(ModelConfig(**entry))
    LOGGER.info("Reset to %d factory model config(s)", len(settings.model_configs))
...
def apply_env_overrides() -> None:
    ...
    for env_var, provider, field in ENV_OVERRIDES:
        # Forward-compat: AIO_ prefix (.env file); backward-compat: legacy unprefixed
        value = os.getenv(f"AIO_{env_var}")
        if value is None:
