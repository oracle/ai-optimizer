...
(function () {
  ...
  var useXHR =
    XMLHttpRequest && 'prototype' in XMLHttpRequest && 'withCredentials' in XMLHttpRequest.prototype;
    ...
    forEach(str.split(sep), function (entry) {
      if (entry !== '') {
        ...
        obj[decodeURIComponent(ref[0])] =
          ref[1] != null ? decodeURIComponent(ref.slice(1).join(eq)) : undefined;
      }
    });
    return obj;
  };
  ...
  var getColorSchemeMediaQuery = function (systemColorScheme, widgetColorScheme) {
    return (
      ...
      '){' +
      widgetColorSchemes[
        hasOwnProperty(widgetColorSchemes, widgetColorScheme) ? widgetColorScheme : systemColorScheme
      ] +
      '}'
    );
  };
  ...
  var render$1 = function (root, options, func) {
    ...
    if (
      ...
      (hostname === domain &&
        path[3] === 'releases' &&
        (path[4] === 'download' || (path[4] === 'latest' && path[5] === 'download'))) ||
