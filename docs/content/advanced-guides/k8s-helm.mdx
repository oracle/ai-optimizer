...
The chart can expose the built-in development provider from the Server pod;
it does not deploy a separate identity-service image. It requires a configured
CORE Oracle Database and is for development or evaluation, not production.

Give the provider a dedicated HTTPS hostname and route that hostname to the
provider listener. The issuer is the root URL, so do not add an `/oidc` path.
...
server:
  devOidc:
    ...
    webClientRedirectUri: https://optimizer.example.com/oauth2callback
    passwordAutoGenerate: true
    ingress:
      enabled: true
      className: nginx
      host: auth.example.com
      tlsSecretName: optimizer-auth-tls
```

This configures `AIO_AUTH_MODE=dev`, exposes port `8765` inside the
Server pod, and mounts a Streamlit OIDC configuration that exposes only the
API access token. The Client callback URL must use the Client's public origin
and end with `/oauth2callback`. The provider ingress terminates TLS and
forwards requests to port `8765`.

To provide a stable bootstrap password, create a Kubernetes Secret and set
`server.devOidc.passwordSecretName` and `server.devOidc.passwordSecretKey`.
The value is applied to the bootstrap account whenever the Server starts.
Alternatively, set `server.devOidc.passwordAutoGenerate=true` for a
chart-managed retained Secret. The chart provides that value only to the Server
as `AIO_AUTH_DEV_ADMIN_PASSWORD`. You can also provide
`AIO_AUTH_DEV_ADMIN_PASSWORD` through `server.envSecret.content` or a prebuilt
Secret referenced by `server.envSecret.secretName`. One of these sources is
required when the provider is enabled; Helm rejects a configuration that would
leave the bootstrap password unretrievable.
...
#### Reuse an OBaaS ADB-S Wallet

When AI Optimizer is installed as an OBaaS subchart and both applications use
the same `ADB-S`, OBaaS owns the `AutonomousDatabase` resource and its wallet
Secrets. Configure AI Optimizer to reuse those resources instead of creating
another set. In the OBaaS values file, set both `useExisting: true` and
`existingWalletSource: obaas`:
  ...
  server:
    database:
      ...
      adb:
        useExisting: true
        existingWalletSource: obaas
        serviceName: <adb-tns-alias>
      privAuthn:
        secretName: <shared-privileged-database-secret>
```

`existingWalletSource: obaas` takes effect only with `useExisting: true`.
With `useExisting: true` and no source selection, AI Optimizer expects
pre-existing wallet resources that use its default naming convention. With
`useExisting: false`, it creates and manages those resources using the same
default names. Explicit wallet Secret names take precedence when configured.
