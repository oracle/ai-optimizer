...
### Embedded Authentication Gateway

The chart exposes the embedded authentication gateway from the Server pod. It
uses CORE and does not deploy a separate identity-service image.
...
This configures `AIO_AUTH_MODE=local`, exposes port `8765` inside the Server
pod, and mounts a Streamlit OIDC configuration that exposes the opaque API
access token. The Client callback URL must use the Client's public origin and
end with `/oauth2callback`. The gateway ingress terminates TLS and forwards
requests to port `8765`.

To provide a stable bootstrap password, create a Kubernetes Secret and set
`server.auth.passwordSecretName` and `server.auth.passwordSecretKey`.
Alternatively, set `server.auth.passwordAutoGenerate=true` for a chart-managed
retained Secret. The chart provides that value only to the Server as
`AIO_AUTH_LOCAL_ADMIN_PASSWORD`.

For GitHub or external OIDC, set `server.auth.mode` to `github` or `oidc` and
provide the corresponding `AIO_AUTH_GITHUB_*` or `AIO_AUTH_OIDC_*` credentials
through `server.envSecret`. The ingress and Streamlit configuration remain the
same. See [Access Control](../guides/access-control.mdx) for the provider
