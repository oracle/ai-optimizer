...
import ReleaseSourceExtract from '../_assets/partials/release-source-extract.mdx';
...
When two examples use the same top-level key, such as `server:`, add their settings to the existing section rather than duplicating the key. The chart's complete [`values.yaml`](https://github.com/oracle/ai-optimizer/blob/main/helm/values.yaml) lists every available option.
...
The extracted release also includes complete local examples in `helm/examples/values-kind-sidb-free.yaml` and `helm/examples/values-kind-adb-free.yaml`.
...
For a non-OKE installation, the helper below generates and applies an `oci-config-file` Kubernetes Secret from your OCI config file:
...
Install the published chart using the values file in the extracted release directory:
...
For public Kubernetes access, terminate TLS at an ingress or gateway. The [Helm chart ingress TLS example](https://github.com/oracle/ai-optimizer/tree/main/helm#example-6-ingress-with-tls) shows how to create a Kubernetes TLS Secret and reference it from the Server and Client ingress values.
