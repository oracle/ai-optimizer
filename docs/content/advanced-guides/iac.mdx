...
import {GitHubSourceLink, OciResourceManagerStackLink} from '@site/src/components/GitHubSourceLink';

The **AI Optimizer** includes [OpenTofu](https://opentofu.org/) infrastructure as code (**IaC**) in the <GitHubSourceLink path="opentofu" type="tree"><code>opentofu/</code></GitHubSourceLink> directory for deploying into Oracle Cloud Infrastructure (OCI). You can either deploy the IaC from [OCI Resource Manager](https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm) or from a local checkout using OpenTofu or Terraform.
...
Use OpenTofu or Terraform 1.5 or later when deploying from a local checkout. Before continuing, follow the <GitHubSourceLink path="opentofu" type="tree">OpenTofu README</GitHubSourceLink> to configure prerequisites and OCI credentials. Then start from the example closest to your deployment:

| Example                                                                                                                    | Deployment                              |
| -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| <GitHubSourceLink path="opentofu/examples/vm-new-adb.tfvars"><code>vm-new-adb.tfvars</code></GitHubSourceLink>             | VM with a new Autonomous Database       |
| <GitHubSourceLink path="opentofu/examples/vm-byo-adb.tfvars"><code>vm-byo-adb.tfvars</code></GitHubSourceLink>             | VM with an existing Autonomous Database |
| <GitHubSourceLink path="opentofu/examples/vm-byo-other-db.tfvars"><code>vm-byo-other-db.tfvars</code></GitHubSourceLink>   | VM with another Oracle database         |
| <GitHubSourceLink path="opentofu/examples/k8s-new-adb.tfvars"><code>k8s-new-adb.tfvars</code></GitHubSourceLink>           | OKE with a new Autonomous Database      |
| <GitHubSourceLink path="opentofu/examples/k8s-byo-other-db.tfvars"><code>k8s-byo-other-db.tfvars</code></GitHubSourceLink> | OKE with another Oracle database        |
| <GitHubSourceLink path="opentofu/examples/always-free.tfvars"><code>always-free.tfvars</code></GitHubSourceLink>           | Always Free VM and Autonomous Database  |
