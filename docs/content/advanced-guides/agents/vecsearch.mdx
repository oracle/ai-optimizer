...
- When a small model is selected in the GUI, its first CPU-optimization pass disables Store Discovery, Rephrase, and Grade. You can re-enable these options; API and persisted settings are otherwise used as configured. See [Model Configuration](../../guides/ai-models#cpu-optimization) for details.
