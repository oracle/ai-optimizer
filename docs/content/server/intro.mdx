...
The service layer of the **AI Optimizer** is the API Server, referred to as the **AI Optimizer Server**. By default, the **AI Optimizer Server** starts on port 8000.
...
Powered by [FastAPI](https://fastapi.tiangolo.com/) and [Uvicorn](https://uvicorn.dev/), the **AI Optimizer Server** hosts the REST API, agent and flow runtime, and MCP server. It connects REST-capable clients to AI models, configured tools, and the Oracle AI Database.

## [Calling the API](./api-examples/intro)
...
Several web clients, IDEs, MCP clients, and external applications can use the same server. The server keeps their working sessions separate while they use the same configured resources.
