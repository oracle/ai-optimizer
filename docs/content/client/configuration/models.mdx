...
The **AI Optimizer** can connect to language and embedding models supported by [LiteLLM](https://docs.litellm.ai/docs/providers) for language-model generation and vector creation. You can add, modify, and delete model configurations from the client.

At a minimum, a _language model_ must be configured for the LLM-only Agent and the model calls used by tool-enabled routes. For Retrieval-Augmented Generation (**RAG**), an _embedding model_ will also need to be configured.

:::note[Choose a Tool-Calling Model]
Some older and small _language models_ may not have native function or tool-calling support for the NL2SQL Agent and model-driven Vector Search steps, which may result in unexpected behavior.
:::
