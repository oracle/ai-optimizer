...
The **AI Optimizer** MCP server can be consumed by external MCP clients. The web client provides tailored configurations for MCP Inspector, Cline for VS Code, LangGraph, and Claude Desktop. For another client that accepts Streamable HTTP, select **Generic Streamable HTTP**.
