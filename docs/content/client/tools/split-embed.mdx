---
title: 📚 Split/Embed
description: Split and embed documents for vector search.
sidebar_position: 10
---
...
The first phase in building a Retrieval-Augmented Generation (RAG) experience with the Vector Search flow is document splitting and embedding. During this phase, source documents are divided into chunks, vector embeddings are generated for each chunk, and the resulting embeddings are stored in a Vector Store.

At query time, relevant chunks are retrieved using [Vector Search](../chatbot#vector-search) and injected into the model context to produce grounded answers based on the provided information.
...
To enable embedding models, configure them in **Configuration** > **Models**. See [Model Configuration](../configuration/models) for details.
...
The **Create New Vector Store** option allows you to create a new vector store table and populate it with embeddings generated from one or more data sources. If compatible vector stores already exist, you can disable this option and add content to one of them. See [Edit Existing Vector Store](#edit-existing-vector-store) for details.
...
- **Knowledge Base Source**: specifies the origin of the documents to be embedded. Supported sources include:
  ...
  - **SQL**: run a read-only `SELECT` query against an Oracle Database and embed its results. The query results are staged as a CSV document before they are processed. When using this option, provide:
    - **Database**: select the target database from the list of configured and connected databases (see [Database Configuration](../configuration/databases));
...
Use the **Description** field to say what the store contains and when it should be used. This is especially useful with **Store Discovery**, where the description helps the language model select the most relevant vector store for a question.
