...
description: Configure Deep Data Security.
sidebar_position: 15
---
...
![Deep Data Security](../../_assets/images/dds.png)

:::note[Requires Oracle AI Database 26ai]
Deep Data Security is available in Oracle AI Database 26ai. When the selected database does not support it, the **Deep Data Security** tab displays a warning.
:::

## Prerequisites

The configured database user needs the [Deep Data Security privileges](../../guides/database#deep-data-security-privileges). The tool reads the user's privileges and enables only the actions that are permitted; anything the user is not privileged to do is disabled.

## Using Deep Data Security

Navigate to **Tools** > **Deep Data Security**. It is organized into three sections:
...
1. Create a local [**data role**](#data-roles).
   ...
   ![DDS - Create Data Role](../../_assets/images/dds-create-data-role.png)

1. Create an [**end user**](#end-users) and assign the data role to it.
   
   ![DDS - Create Data User](../../_assets/images/dds-create-data-user.png)

1. If more than one end user exists, select the end user in **Connect tools as**.
   
   ![DDS - Select Data User](../../_assets/images/dds-select-data-user.png)

1. Create a [**data grant**](#data-grants) for that role, specifying the tables or views, privileges, and any column or row restrictions.
   
   ![DDS - Create Data Grant](../../_assets/images/dds-create-data-grant.png)

1. Use Vector Search or NL2SQL and enable **Deep Data Security** to confirm that the tools return only the data allowed by the grant.
   
   ![DDS - Enable on Chatbot](../../_assets/images/dds-enable.png)
...
In the **AI Optimizer**, the configured database user, for example `DEMO`, owns the application objects, vector stores, and any tables it creates. When _Deep Data Security_ is not enabled, the _Vector Search_ and _NL2SQL_ tools connect as `DEMO` with full access to the data in the schema.
