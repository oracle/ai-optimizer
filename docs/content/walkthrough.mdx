...
This walkthrough will guide you through a basic installation of the **Oracle AI Optimizer and Toolkit** (the **AI Optimizer**). It will allow you to experiment with tool-enabled agents and retrieval workflows using Retrieval-Augmented Generation (**RAG**) and Natural Language to SQL (**NL2SQL**) with the Oracle AI Database at the core.
...
- Vector Store - Oracle AI Database Free
- The **AI Optimizer**

### Language Model - granite4.1:8b

To start the _Chatbot_, configure a language model. The walkthrough will use [Ollama](https://ollama.com/) to run the _granite4.1:8b_ model.
...
The **AI Optimizer** provides a web client for experimenting with language-model parameters, agent routes, and **RAG**.
...
AI Vector Search in Oracle AI Database provides the ability to store and query private business data using a natural language interface. The **AI Optimizer** uses these capabilities to ground model responses via Retrieval-Augmented Generation (**RAG**). [Oracle AI Database Free](https://www.oracle.com/database/free/get-started/) provides an ideal, no-cost vector store for this walkthrough.

<OracleAiDatabaseFree />
   5. Create a [database user](./guides/database#database-user) for the walkthrough:
...
In a web browser, navigate to [`http://localhost:8501`](http://localhost:8501):
![Chatbot](_assets/images/chatbot-no-models.png)
...
1. Enable the `granite4.1:8b` model that you pulled earlier by clicking the _Edit_ button
   ![Configure Language Model](_assets/images/models-edit.png)
1. Tick the _Enabled_ checkbox, leave all other settings as-is, and _Save_
   ![Enable Language Model](_assets/images/models-enable-llm.png)
...
![Say Hello?](_assets/images/chatbot-say-hello.png)
...
The `Chat Model:` field will have been pre-set to the only enabled language model and the chat input will be ready for your first request.
...
1. Enable the `mxbai-embed-large` embedding model following the same process as you did for the language model.
   ![Configure Embedding Model](_assets/images/models-edit-embed.png)
...
![Configure Database](_assets/images/database-config.png)
...
1. Change the **Knowledge Base Source** to `Web`
   ![Split and Embed](_assets/images/split-embed-web.png)
1. Enter the URL and press Enter:
   ![Split and Embed Web Pages](_assets/images/split-embed-web-pages.png)
   ```text
   https://docs.oracle.com/en/database/oracle/oracle-database/26/xeinl/oracle-ai-database-free-installation-guide-linux.pdf
   ```

1. Give the Vector Store an Alias: `WALKTHROUGH` and press Enter
   ![Split and Embed Populate](_assets/images/split-embed-populate-walkthrough.png)
...
Responses may vary, but generally the _Chatbot_'s ungrounded response will be inaccurate, including:
...
![Enable RAG](_assets/images/chatbot-enable-vs.png)
...
By asking `Are you sure?`, you are taking advantage of the **AI Optimizer**'s history and context functionality. The Vector Search flow uses the earlier exchange to rephrase the follow-up before retrieving documents. The response should be different and include a list of operating-system packages and maybe even an apology!
...
The [Use Cases](./use-cases/intro) walk you through using your new **AI Optimizer** installation through an end-to-end demo that shows the progressive value of **NL2SQL**, **Vector Search**, and combined-mode grounding a synthetic dataset.
:::

To take your experiments further, consider exploring:

- Turn On/Off/Clear history
- Experiment with different language and embedding models
