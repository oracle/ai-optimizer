...
The AI Optimizer runtime uses a [language model](#language-models) for the LLM-only Agent, the NL2SQL Agent, the Vector Search Flow, and the combined route. To use Retrieval-Augmented Generation (RAG), it also needs an [embedding model](#embedding-models). Configure the selected models in [Configuration > Models](../client/configuration/models).

## Language Models

Language models generate responses and power the model calls in the AI Optimizer's agents and flows. They also support Vector Search steps such as store discovery, document grading, and prompt rephrasing. Choose a model that supports the capabilities your application needs.

### Choose a Language Model with Reliable Tool Use

Tool use is a language model's ability to recognize when it needs an external capability, select the appropriate tool, send valid structured arguments, and use the tool's result in its response. For example, a model might use a database tool to answer a question about your data instead of trying to answer from its existing knowledge.
...
When evaluating language models, first confirm that the provider documents support for function or tool calling. Then test each candidate with representative tasks that require the agent to choose the correct tool, provide valid arguments, handle a tool error, and accurately use the returned result. Prefer the model that performs these tasks consistently in your environment, rather than selecting one based only on general response quality.
...
For more information and instructions on running Ollama on other platforms, see the [Ollama quickstart](https://docs.ollama.com/quickstart).
