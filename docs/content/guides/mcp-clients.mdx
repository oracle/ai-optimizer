...
In a shared API-key deployment, the `X-API-Key` authorizes the holder to invoke the tools published by the AI Optimizer Server. In a principal-authenticated deployment, use the bearer credential issued to the authenticated principal. Provide credentials only to users and systems authorized to use those server-side operations.
...
Start the AI Optimizer Server and obtain the credential for its configured authentication mode. In the included GUI, the API key is available on the [API Server](../client/api_server) page. For standalone shared-key deployments, configure `AIO_API_KEY` before starting the server. See [Environment Variables](./env-vars).
...
For a principal-authenticated deployment, send the bearer credential instead:

```bash
curl -H "Authorization: Bearer $AIO_ACCESS_TOKEN" \
  "http://localhost:8000/mcp/client-config?client=generic"
```
...
The generated configuration is the source of truth: it includes the deployment's URL, any server URL prefix, and the credential header used by the request (`X-API-Key` for shared API-key mode or `Authorization` for principal-authenticated mode).

:::danger[Secrets Inside!]
The configuration contains a credential to access the server, so do not commit or share it.
:::
...
The [MCP Inspector](https://github.com/modelcontextprotocol/inspector) is a developer tool for inspecting MCP servers. In the GUI, select **MCP Inspector** under **Configuration** > **MCP Server** and save the displayed JSON as `mcp.json`. The generated configuration includes the required credential header; import it unchanged.
...
For a shared API-key deployment, confirm that the MCP server is healthy and has registered tools:
