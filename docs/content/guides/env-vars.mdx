...
| Variable      | Description                                                                                                                                                           | Default       |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `AIO_API_KEY` | API key for authenticating requests to the AI Optimizer Server. If unset, a key is generated at startup and displayed on the [API Server](../client/api_server) page. | _(generated)_ |
...
| `AIO_CLIENT_PASSWORD`      | Shared password gating configuration and shared-state controls in the GUI client. This does not replace API Server authentication. See [Access Control](./access-control). | _(none)_    |
...
| Variable                   | Description                                                                                                                                        | Default            |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `AIO_SERVER_URL`           | URL the client uses to reach the API Server. Set it when the client connects to a separately deployed, remote, or reverse-proxied server.                                | _(auto-detected)_  |
...
OpenTelemetry endpoints, protocols, exporters, sampling, and OpenInference payload visibility use their standard integration variables.
...
For Kubernetes, configure the application with Helm values and Kubernetes Secrets. See [Kubernetes](../advanced-guides/kubernetes) and the [Helm chart](https://github.com/oracle/ai-optimizer/tree/main/helm) for the available settings.
