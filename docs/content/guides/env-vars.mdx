...
| Variable | Description | Default |
| --- | --- | --- |
| `AIO_API_KEY` | Shared API key used only when `AIO_AUTH_MODE` is unset. | _(generated)_ |
| `AIO_AUTH_MODE` | Authentication source: `local`, `github`, `oidc`, or `proxy`. A configured CORE database defaults to `local`. | _(none)_ |
| `AIO_AUTH_ISSUER` | Embedded gateway issuer. Plain HTTP is accepted only on loopback. | `http://127.0.0.1:8765` |
| `AIO_AUTH_LISTEN_HOST`, `AIO_AUTH_LISTEN_PORT` | Embedded gateway listener. | `127.0.0.1`, `8765` |
| `AIO_AUTH_WEB_REDIRECT_URI` | Streamlit callback URI, ending in `/oauth2callback`. | _(derived from Client listener)_ |
| `AIO_AUTH_WEB_CLIENT_SECRET` | Confidential credential shared by Streamlit and the gateway. | _(generated)_ |
| `AIO_AUTH_ACCESS_TOKEN_MINUTES` | Opaque API and MCP access-token lifetime. | `15` |
| `AIO_AUTH_LOGIN_SESSION_HOURS` | Gateway browser-session lifetime. | `8` |
| `AIO_AUTH_LOCAL_ADMIN_USERNAME`, `AIO_AUTH_LOCAL_ADMIN_PASSWORD` | Bootstrap local administrator account. | `admin@example.test`, _(generated)_ |
| `AIO_AUTH_GITHUB_CLIENT_ID`, `AIO_AUTH_GITHUB_CLIENT_SECRET` | GitHub OAuth application credentials. | _(none)_ |
| `AIO_AUTH_GITHUB_BASE_URL` | GitHub.com or GitHub Enterprise base URL. | `https://github.com` |
| `AIO_AUTH_GITHUB_ALLOWED_USERS`, `AIO_AUTH_GITHUB_ALLOWED_ORGANIZATIONS` | GitHub IDs or organizations allowed to use the deployment. | _(none)_ |
| `AIO_AUTH_GITHUB_ADMIN_USERS`, `AIO_AUTH_GITHUB_ADMIN_TEAMS` | GitHub IDs or `organization/team-slug` values that grant `aio.admin`. | _(none)_ |
| `AIO_AUTH_OIDC_ISSUER` | Upstream OIDC issuer discovery URL. | _(none)_ |
| `AIO_AUTH_OIDC_CLIENT_ID`, `AIO_AUTH_OIDC_CLIENT_SECRET` | Upstream confidential-client credentials. | _(none)_ |
| `AIO_AUTH_OIDC_SCOPES` | Upstream scopes. | `openid,profile,email` |
| `AIO_AUTH_OIDC_SIGNING_ALGORITHMS` | Upstream ID-token signing algorithms. | `RS256` |
| `AIO_AUTH_OIDC_ROLES_CLAIM`, `AIO_AUTH_OIDC_ALLOWED_CLAIM_VALUES` | Group or role claim and values permitted to use the deployment. | `roles`, _(none)_ |
| `AIO_AUTH_ADMIN_CLAIM_VALUES` | Role or group values that grant `aio.admin`. | `aio.admin` |
...
See [Access Control](./access-control.mdx) for Local, GitHub, and OCI IAM
Identity Domains examples.
