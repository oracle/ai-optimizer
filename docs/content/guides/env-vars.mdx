...
| `AIO_AUTH_DEV_ADMIN_PASSWORD`   | Initial password for the bootstrapped `admin@example.test` development administrator. When unset, a random password is generated at startup for bootstrap. | _(generated)_ |
...
`openid profile email aio.api` scopes. Its initial password is
`AIO_AUTH_DEV_ADMIN_PASSWORD`. When unset, the server generates a random value
at startup; it is used only when creating a missing bootstrap account. HTTP is
supported only on loopback; use a configured HTTPS issuer behind TLS for other
deployments.
