...
| `AIO_AUTH_DEV_WEB_REDIRECT_URI` | Registered Streamlit callback URI for the built-in provider. When unset, the entrypoint derives a browser-facing URI from the Client listener settings.                            | _(derived from Client listener)_      |
...
and callback settings above. If `AIO_AUTH_DEV_WEB_REDIRECT_URI` is unset, its
scheme, address, and port are derived from `AIO_CLIENT_SSL`,
`AIO_CLIENT_ADDRESS`, and `AIO_CLIENT_PORT`. Local and wildcard bind addresses
use `localhost` for the browser callback; other addresses are retained. In an
All-In-One bare-metal installation, when
