...
1. _(Optional)_ Create an [environment file](./guides/env_vars) to pre-configure the application:

   ```bash
   cp src/.env.example src/.env.dev
   ```

   Edit `src/.env.dev` as needed. See [Environment Variables](./guides/env_vars) for details.
...
To get started, review the [IaC](./advanced_guides/iac) documentation.

---
### Kubernetes

The **AI Optimizer** can be deployed into a Kubernetes cluster using a Helm chart. For more information, see the [Kubernetes](./advanced_guides/kubernetes) documentation.
