...
To run the application on bare-metal, download the latest release: [TAR.GZ archive](https://github.com/oracle/ai-optimizer/releases/latest/download/ai-optimizer-src.tar.gz)
...
1. _(Optional)_ Create an [environment file](./env_vars) to pre-configure the application:

   ```bash
   cp src/.env.example src/.env.dev
   ```

   Edit `src/.env.dev` as needed. See [Environment Variables](./env_vars) for details.
...
1. [Configure](./client/configuration/intro) the **AI Optimizer**.
...
The commands in this section work in a macOS/Linux shell or PowerShell. The documentation will reference `podman` commands. If applicable to your environment, `podman` can be substituted with `docker`.

If you are using `docker`, consider aliasing the `podman` command for convenience:

<Tabs>
<TabItem value="macos-linux" label="macOS or Linux">
...
To run the application in a container, download the latest release: [TAR.GZ archive](https://github.com/oracle/ai-optimizer/releases/latest/download/ai-optimizer-src.tar.gz)
...
To get started, review the [IaC](./advanced/iac) documentation.

---
### Kubernetes

The **AI Optimizer** can be deployed into a Kubernetes cluster using a Helm chart. For more information, see the [Kubernetes](./advanced/kubernetes) documentation.
