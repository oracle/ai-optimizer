...
## Step 4: Create the data grant

In **Data Grants**, create the policy that defines the reviewer’s access:

1. Set **Data grant name** to `DG_LEEDS_APPLICATION_SUMMARY`.
2. Set **Object (table/view)** to `MORTGAGE_APPLICATIONS`.
3. Select the `SELECT` privilege.
4. Set **Row predicate** to:

   ```sql
   PROPERTY_LOCATION='Leeds'
   ```

5. Set **Grant to data role** to `MORTGAGE_REVIEWER_ROLE`.
6. Review the generated statement. It should be close to:

   ```sql
   CREATE DATA GRANT DG_LEEDS_APPLICATION_SUMMARY
     AS SELECT ON MORTGAGE_APPLICATIONS WHERE PROPERTY_LOCATION='Leeds'
