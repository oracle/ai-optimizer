# Driver 97 Briefing Pack

Document type: Driver briefing and coaching notes
Driver label: Driver 97
Driver code: Driver097
Team: Velocity Forge
...
Driver 97 is running with a Low Drag baseline on Rig 22. The profile remains Consistency Focused, which fits the current Velocity Forge programme as long as the driver links style to measurable outputs: lap-time spread, tyre deg, pit timing, and mistakes under pressure.

## Coaching Notes

Main priority: Qualifying Pace. The coach is most concerned about lap-time consistency after overtaking attempts. Next session should include two jobs: try a shorter first stint to protect track position; then review the final-sector trace before the next session. Out-lap calls need to be brief and made before the lap time falls away.
