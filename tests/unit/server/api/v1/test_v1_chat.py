...
class TestChatPost:
    ...
    @pytest.mark.asyncio
    @patch("server.api.v1.chat.utils_chat.completion_generator")
...
class TestChatHistoryClean:
    ...
    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_clean_success(self, mock_graph_main):
        ...
        mock_graph_main.assert_called_once_with(tools=[])

    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_clean_updates_state_correctly(self, mock_graph_main):
    ...
    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_clean_handles_key_error(self, mock_graph_main):
        """chat_history_clean should handle KeyError gracefully."""
        mock_agent = MagicMock()
        mock_agent.update_state = MagicMock(side_effect=KeyError("thread not found"))
        mock_graph_main.return_value = mock_agent
    ...
    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_clean_uses_correct_thread_id(self, mock_graph_main):
...
class TestChatHistoryReturn:
    ...
    async def test_chat_history_return_success(self, mock_convert, mock_graph_main):
        """chat_history_return should return chat messages with response_metadata preserved."""
        # Create mock messages - one with response_metadata, one without
        mock_msg_with_metadata = MagicMock()
        mock_msg_with_metadata.response_metadata = {"token_usage": {"total_tokens": 100}}

        mock_msg_without_metadata = MagicMock()
        mock_msg_without_metadata.response_metadata = {}

        mock_messages = [mock_msg_without_metadata, mock_msg_with_metadata]
        ...
        # Mock convert_to_openai_messages to return a dict for each message
        ...
        assert len(result) == 2
        # First message has no response_metadata (empty dict is falsy)
        assert "response_metadata" not in result[0]
        # Second message should have response_metadata preserved
        assert result[1]["response_metadata"] == {"token_usage": {"total_tokens": 100}}
        mock_graph_main.assert_called_once_with(tools=[])

    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_return_handles_key_error(self, mock_graph_main):
        """chat_history_return should handle KeyError gracefully."""
        mock_agent = MagicMock()
        mock_agent.get_state = MagicMock(side_effect=KeyError("thread not found"))
        mock_graph_main.return_value = mock_agent
    ...
    @pytest.mark.asyncio
    @patch("server.api.v1.chat.graph.main")
    async def test_chat_history_return_uses_correct_thread_id(self, mock_graph_main):
    ...
    @patch("server.api.v1.chat.graph.main")
    @patch("server.api.v1.chat.convert_to_openai_messages")
    async def test_chat_history_return_empty_history(self, mock_convert, mock_graph_main):
    ...
    @patch("server.api.v1.chat.graph.main")
    @patch("server.api.v1.chat.convert_to_openai_messages")
    async def test_chat_history_return_preserves_response_metadata(self, mock_convert, mock_graph_main):
        """chat_history_return should preserve response_metadata for AI messages."""
        # Create mock AI message with response_metadata
        mock_ai_msg = MagicMock()
        mock_ai_msg.response_metadata = {
            "token_usage": {"prompt_tokens": 10, "completion_tokens": 20},
            "vs_metadata": {"searched_tables": ["table1"]},
        }

        mock_state = MagicMock()
        mock_state.values = {"messages": [mock_ai_msg]}

        mock_agent = MagicMock()
        mock_agent.get_state = MagicMock(return_value=mock_state)
        mock_graph_main.return_value = mock_agent

        mock_convert.side_effect = [[{"role": "assistant", "content": "Response"}]]

        result = await chat.chat_history_return(client="test_client")

        assert len(result) == 1
        assert "response_metadata" in result[0]
        assert result[0]["response_metadata"]["token_usage"]["prompt_tokens"] == 10
        assert result[0]["response_metadata"]["vs_metadata"]["searched_tables"] == ["table1"]
