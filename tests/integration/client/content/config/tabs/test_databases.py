...
class TestStreamlit:
    ...
    def test_no_database(self, app_server, app_test):
        ...
        at = app_test(self.ST_FILE).run()
        assert at.session_state.database_configs is not None
        at.text_input(key="database_user").set_value(TEST_DB_CONFIG["db_username"]).run()
        at.text_input(key="database_password").set_value(TEST_DB_CONFIG["db_password"]).run()
        at.text_input(key="database_dsn").set_value(TEST_DB_CONFIG["db_dsn"]).run()
        at.button(key="save_database").click().run()
    ...
    def test_connected(self, app_server, app_test, db_container):
        ...
        assert at.session_state.database_configs[0]["user"] == TEST_DB_CONFIG["db_username"]
        assert at.session_state.database_configs[0]["password"] == TEST_DB_CONFIG["db_password"]
        assert at.session_state.database_configs[0]["dsn"] == TEST_DB_CONFIG["db_dsn"]
