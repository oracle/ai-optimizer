...
class TestStreamlit:
    ...
    def test_copy_client_settings_success(self, app_test, app_server):
        ...
        def settings_equal_ignoring_client(s1, s2):
            """Compare settings while ignoring the 'client' field which is expected to differ."""
            s1_copy = {k: v for k, v in s1.items() if k != "client"}
            s2_copy = {k: v for k, v in s2.items() if k != "client"}
            return s1_copy == s2_copy

        # Check that Server/Client Identical (excluding 'client' field)
        assert settings_equal_ignoring_client(
            at.session_state.client_settings, at.session_state.server_settings
        )
        # Update Client Settings
        at.session_state.client_settings["oci"]["auth_profile"] = "TESTING"
        assert not settings_equal_ignoring_client(
            at.session_state.client_settings, at.session_state.server_settings
        )
