...
class TestStreamlit:
    ...
    def test_copy_user_settings_success(self, app_test, app_server):
        """Test the copy_user_settings function with a successful API call"""
        assert app_server is not None
        at = app_test(self.ST_FILE).run()

        # Check that Server/Client Identical
        assert at.session_state.user_settings == at.session_state.server_settings
        # Update Client Settings
        at.session_state.user_settings["oci"]["auth_profile"] = "TESTING"
        assert at.session_state.user_settings != at.session_state.server_settings
